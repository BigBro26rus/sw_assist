...
function renderSkillModal(searchTerm = '') {
    ...
    container.innerHTML = filtered.map(skill => {
        ...
        if (showAllSkills && existingSkill) {
            skillInfo = `<div style="color: #667eea; font-weight: bold; margin-top: 5px;">Текущее значение: ${currentValue} (характеристика: ${charValue})</div>`;
        }
        ...
        container.innerHTML = filtered.map(flaw => {
            ...
            return `
                <div class="modal-item" data-uuid="${flaw.uuid}" data-can-remove="${canRemove}">
                    ...
                    ${!canRemove ? '<div style="color: #dc3545; margin-top: 5px;">Можно удалить только МЕЛКИЙ изъян или уменьшить КРУПНЫЙ до МЕЛКОГО</div>' : ''}
                </div>
            `;
        }).join('');
...
function renderTraitModal() {
    ...
    if (availableTraits.length > 0) {
        html += '<div class="traits-section-header" style="margin-top: 0; margin-bottom: 10px; font-weight: bold; color: #28a745; font-size: 1.1em;">✓ Доступные черты</div>';
    ...
    if (unavailableTraits.length > 0) {
        if (availableTraits.length > 0) {
            html += '<div class="traits-section-header" style="margin-top: 30px; margin-bottom: 10px; font-weight: bold; color: #dc3545; font-size: 1.1em;">✗ Недоступные черты</div>';
        }
        html += unavailableTraits.map(({ trait, reasons }) => {
            return `
                <div class="modal-item trait-unavailable" data-uuid="${trait.uuid}">
                    ...
                    <div class="modal-item-description">${trait.description}</div>
                    <div class="trait-requirements-block" style="margin-top: 10px; padding: 8px; background: #fff3cd; border-left: 3px solid #ffc107; border-radius: 3px;">
                        <div style="font-weight: bold; color: #856404; margin-bottom: 5px;">Требования не выполнены:</div>
    ...
    if (html === '') {
        html = '<p style="color: #666; padding: 20px; text-align: center;">Черты не найдены</p>';
    }
...
function saveCharacter() {
    ...
    try {
        ...
        alert('Персонаж успешно сохранён! Файл скачан.');
    } catch (error) {
...
function renderEquipment() {
    ...
    if (items.length === 0) {
        container.innerHTML = '<p style="color: #666; font-style: italic; padding: 10px;">Инвентарь пуст</p>';
        return;
    }
...
function renderAdvancements() {
    ...
    if (character.advancements.length === 0) {
        container.innerHTML = '<p style="color: #666; font-style: italic; padding: 10px;">Нет доступных повышений</p>';
        ...
        return `
            <div class="advancement-card">
                <div class="advancement-info">
                    ...
                    ${description ? `<div style="margin-top: 5px; color: #555; font-size: 0.9em;">${description}</div>` : ''}
                </div>
...
function renderSkillModalForAdvancement(mode) {
    ...
    if (filtered.length === 0) {
        container.innerHTML = `<p style="color: #666; padding: 20px; text-align: center;">
            ${mode === 'one' 
